...
# Suponiendo que tienes un DataFrame 'df' y quieres filtrar por la columna 'columna_filtro'
filtro = dataset['tipo'] == 'Finca'  # Define tu condición de filtro aquí
cantidad_registros_filtrados = len(dataset[filtro])

print("Cantidad de registros después de aplicar el filtro:", cantidad_registros_filtrados)
...
# Suponiendo que tienes un DataFrame 'df' y quieres filtrar por la columna 'columna_filtro'
filtro = df_sin_duplicados['tipo'] == 'Finca'  # Define tu condición de filtro aquí
cantidad_registros_filtrados = len(df_sin_duplicados[filtro])

print("Cantidad de registros después de aplicar el filtro:", cantidad_registros_filtrados)

# COMMAND ----------

Q1 = df_sin_duplicados['precio'].quantile(0.25)
Q3 = df_sin_duplicados['precio'].quantile(0.75)
IQR = Q3 - Q1
...
Q1 = np.percentile(df_sin_duplicados['precio'], 25)
Q3 = np.percentile(df_sin_duplicados['precio'], 75)
IQR = Q3 - Q1
limite_inferior = Q1 - 1.5 * IQR
limite_superior = Q3 + 1.5 * IQR

# Identificar outliers en la columna de interés
# outliers = df_sin_duplicados[(df_sin_duplicados['precio'] < limite_inferior) | (df_sin_duplicados['precio'] > limite_superior)]
print(Q1, Q3, IQR, limite_inferior,limite_superior)

# COMMAND ----------

# código para conocer la distribución estadistica de los datos, por cada atributo
summary = df_sin_duplicados['precio'].describe()
summary = summary.transpose()
summary
...
condition1 = df_sin_duplicados['precio'] < lower_bound
condition2 = df_sin_duplicados['precio'] > upper_bound
...
sns.set_style("whitegrid")  # Estilo de la trama

# Crear el boxplot con Seaborn
sns.boxplot(x=df_filtered['precio'], color='skyblue')

plt.title("Diagrama de Caja de Bigotes de Precios", fontsize=16)
...
# Suponiendo que tienes un DataFrame 'df' con la columna 'precio'
precio_data = df_filtered['precio']

# Calcular la media y la mediana
mean_price = precio_data.mean()
median_price = precio_data.median()

# Crear el histograma
plt.figure(figsize=(10, 6))
plt.hist(precio_data, bins=30, color='skyblue', edgecolor='black', alpha=0.7)
...
plt.title('Histograma de la Variable Precio con Media y Mediana')
plt.legend()
...
def asignar_grupo(valor):
    valor = valor/1000000
    if 10 <= valor < 700:
        grupo_inicio = (valor // 20) * 20
        grupo_fin = grupo_inicio + 20
    elif 700 <= valor < 1200:
        grupo_inicio = 700 + ((valor - 700) // 50) * 50
        grupo_fin = grupo_inicio + 50
    elif 1200 <= valor < 3000:
        grupo_inicio = 1200 + ((valor - 1200) // 100) * 100
        grupo_fin = grupo_inicio + 100
    else:
        grupo_inicio = 3000 + ((valor - 3000) // 200) * 200
        grupo_fin = grupo_inicio + 200
       
    return f"{grupo_inicio}M - {grupo_fin}M"
 

 
# Aplicar la función a la columna 'valor'
df_filtered['Grupo_Precio'] = df_filtered['precio'].apply(asignar_grupo)
...
total_instance_per_class = df_filtered['Grupo_Precio'].value_counts()
total_instance_per_class

# COMMAND ----------

cantidad_valores_unicos = df_filtered['Grupo_Precio'].nunique()
...
plt.figure(figsize=(10, 6))
plt.hist(df_filtered['Grupo_Precio'], bins=10, color='skyblue', edgecolor='black')
plt.xlabel('Valores')
plt.ylabel('Frecuencia')
...
EPOCHS = 100
history = modelDL.fit(X_train, y_train, epochs=EPOCHS, batch_size=32, validation_data=(X_test, y_test), callbacks=[callbacks])
...
precision = precision_score(y_test, predictionsRF, average=average_method)
recall = recall_score(y_test, predictionsRF, average=average_method)
f1 = f1_score(y_test, predictionsRF, average=average_method)
...
# Calcular métricas
average_method = 'macro'
accuracy = accuracy_score(y_test, predictionsSVM)
precision = precision_score(y_test, predictionsSVM, average=average_method)
recall = recall_score(y_test, predictionsSVM, average=average_method)
f1 = f1_score(y_test, predictionsSVM, average=average_method)
# Imprimir las métricas
print("Accuracy:", accuracy)
print("Precision:", precision)
print("Recall:", recall)
...
precision = precision_score(y_test, predictionsXG, average=average_method)
recall = recall_score(y_test, predictionsXG, average=average_method)
f1 = f1_score(y_test, predictionsXG, average=average_method)
# Imprimir las métricas
print("Accuracy:", accuracy)
print("Precision:", precision)
print("Recall:", recall)
